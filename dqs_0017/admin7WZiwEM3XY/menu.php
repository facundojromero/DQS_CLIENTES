...
// Render del menú para GET y para POSTs que no sean del toggle
    ...
    $whatsapp_enabled = dqs_admin_can_use_whatsapp($conn);
    $regalos_enabled = dqs_admin_can_use_gifts($conn);
    $contactos_envio_enabled = dqs_admin_can_view_contactos_envio($conn);

    $invitados_menu = [
        "Lista de invitados" => "?new=invitados", // Solo link
    ];

    // En Oro + FORM, el alta visible debe ir al staging pre_*; el contexto
    // Oro + CÓDIGO conserva el alta histórica definitiva en invitados*.
    if ($contactos_envio_enabled) {
        $invitados_menu["Cargar contacto de envío"] = "?new=contactos_envio&accion=nuevo";
    } else {
        $invitados_menu["Nuevo invitado"] = "?new=invitados&nuevo=0";
    }

    if ($whatsapp_enabled) {
        $invitados_menu["Enviar Invitaciones"] = "?new=envioinvitaciones"; // Solo link
    }

    if ($contactos_envio_enabled) {
        $invitados_menu["Contactos de envío"] = "?new=contactos_envio";
    }

    $invitados_menu["Exportar Invitados"] = "exportar_invitados.php"; // Solo link

    $menu_structure = [
        "Inicio" => "?new=inicio&", // Solo link (elemento principal sin submenú)        
        "Invitados" => $invitados_menu,        
        ...
        "Web" => [
            ...
            "Contactar" => ["db_section" => "contact", "url_name" => "?new=contacto"]    // Link Y Toggle
        ],
        "Datos" => "?new=datos", // Solo link (elemento principal sin submenú)
        "Configuración RSVP" => "?new=rsvp_config", // Solo link (elemento principal sin submenú)
     
        "Cerrar Sesión" => "logout.php" // Solo link (elemento principal sin submenú)
    ];

    if ($regalos_enabled) {
        $menu_structure = array_slice($menu_structure, 0, 1, true)
            + [
                "Regalos" => [
                    "Confirmar" => "?new=ventas&view=confirmarPago",
                    "Recibidos" => "?new=ventas&view=yaConfirmados",
                    "Lista de regalos" => "?new=regalos",
                ],
            ]
            + array_slice($menu_structure, 1, null, true);
    }
