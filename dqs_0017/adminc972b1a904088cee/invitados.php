...
GROUP BY
 ...
 SELECT
     id_invitados,
     COUNT(*) AS cantidad_personas_mesa,
     GROUP_CONCAT(
         CASE
             WHEN TRIM(COALESCE(alimento, '')) <> ''
               AND LOWER(TRIM(alimento)) <> 'no'
               AND LOWER(TRIM(alimento)) <> 'ninguna'
             THEN CONCAT(
                 HEX(COALESCE(nombre_invitado, '')), ':',
                 HEX(TRIM(alimento)), ':',
                 HEX(TRIM(COALESCE(alimento_comentario, '')))
             )
         END
         ORDER BY id ASC SEPARATOR '|'
     ) AS restricciones_alimentarias
 FROM
     invitados_listado_mesa
 GROUP BY
     id_invitados
 ) i
 ON a.id = i.id_invitados
WHERE 1=1
and a.activo < 2
...
// NUEVA LÓGICA: Si se selecciona 'Discrepancia', no aplicamos el filtro normal de 'confirmacion'
if ($discrepanciaFiltro == 'Discrepancia') {
    ...
    // Si no se selecciona 'Discrepancia', aplicamos el filtro de confirmación normal
...
<body>
 ...
 <option value="Discrepancia" <?php if ($discrepanciaFiltro == 'Discrepancia') echo 'selected'; ?>>Discrepancia</option>
 </select>
 </div>
 <div class="search-item">
...
// Arma el detalle por persona desde invitados_listado_mesa. Los valores llegan
// codificados en hexadecimal para que nombres o comentarios con separadores no
// alteren la estructura generada por GROUP_CONCAT.
function renderizarRestriccionesAlimentarias($row) {
    $restricciones = [];

    if (!empty($row['restricciones_alimentarias'])) {
        foreach (explode('|', $row['restricciones_alimentarias']) as $restriccionCodificada) {
            $campos = explode(':', $restriccionCodificada, 3);
            if (count($campos) !== 3) {
                continue;
            }

            $nombre = hex2bin($campos[0]);
            $alimento = hex2bin($campos[1]);
            $comentario = hex2bin($campos[2]);
            if ($nombre === false || $alimento === false || $comentario === false) {
                continue;
            }
    ...
    // Compatibilidad con registros anteriores que todavía no tienen personas
    // asociadas en invitados_listado_mesa.
    if (empty($restricciones) && (int) ($row['cantidad_personas_mesa'] ?? 0) === 0) {
        $alimentoLegacy = trim((string) ($row['alimento'] ?? ''));
        if ($alimentoLegacy !== '' && !in_array(strtolower($alimentoLegacy), ['no', 'ninguna'], true)) {
            $restricciones[] = [
                'nombre' => trim($row['nombre'] . ' ' . $row['apellido']),
                'alimento' => $alimentoLegacy,
                'comentario' => trim((string) ($row['confirmacion_comentario'] ?? '')),
            ];
        }
    }

    $html = "<div class='restricciones-alimentarias'>"
        . "<p><i class='fas fa-utensils'></i> <strong>Restricciones alimentarias:</strong></p>";

    if (empty($restricciones)) {
        return $html . "<p>Sin restricciones</p></div>";
    }

    $html .= '<ul>';
    foreach ($restricciones as $restriccion) {
        $nombre = htmlspecialchars($restriccion['nombre'], ENT_QUOTES, 'UTF-8');
        $alimento = htmlspecialchars($restriccion['alimento'], ENT_QUOTES, 'UTF-8');
        $comentario = htmlspecialchars($restriccion['comentario'], ENT_QUOTES, 'UTF-8');
        $html .= "<li><strong>{$nombre}:</strong> {$alimento}";
        if ($comentario !== '') {
            $html .= " — {$comentario}";
        }
        $html .= '</li>';
...
// Esto nos lleva de "/regalos_casamiento_v2/adminA0KxlHeVGc/invitados.php" a "/regalos_casamiento_v2"
...
$mensaje = urlencode("Hola {$row['invitados']}, te estamos esperando. Tu codigo es {$row['codigo']}. \nMirá tu invitación: $invitacion_url_final");
...
foreach ($telefonos as $tel) {
    ...
    if ($tel_sin_signo !== '') {
        $links[] = "<a href='https://wa.me/54{$tel_sin_signo}?text={$mensaje}' target='_blank'>{$tel}</a>";
    }
}
...
// --- FIN CÓDIGO WHATSAPP AÑADIDO ---
 ...
 $restricciones_alimentarias_html = renderizarRestriccionesAlimentarias($row);
...
<div class='invitacion'>
     <a href='invitaciones/{$row['id_imagen']}.jpg' target='_blank' class='invitacion-link'>
         <p><i class='fas fa-envelope'></i> Tarjeta de invitación</p>
     </a>
</div>
 ...
 {$restricciones_alimentarias_html}
...
<script>
    ...
    function applyFilter() {
        ...
        // La lógica para la confirmación y la discrepancia se maneja aquí
        if (confirmation) {
            // Si el valor es 'Discrepancia', lo agregamos como un filtro separado
    ...
    // Evento para aplicar filtros al escribir en el campo de búsqueda
    document.getElementById('searchInput').addEventListener('keyup', function() {
        applyFilter();
    });

    // Evento para ordenar la lista de invitados al cargar el DOM
    document.addEventListener('DOMContentLoaded', function() {
        const guestList = document.getElementById('guestList');
        const guests = Array.from(guestList.getElementsByClassName('grid-item'));

        // Ejemplo de ordenación por nombre
        guests.sort((a, b) => {
    ...
    // Función para editar un invitado y actualizar la URL
...
<script>
    // Evento para manejar el toggle de la barra de navegación
    document.addEventListener('DOMContentLoaded', function() {
