...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $precio_producto_base = $row["precio"];
        $precio_producto_a_mostrar = $precio_producto_base;
        if ($currency == 2) { // Si la moneda es Dólares
        ...
        $productos[] = $row["titulo"] . " (Cantidad: " . $row["cantidad"] . ", Subtotal: " . ($currency == 2 ? "u\$s " : "$ ") . number_format($subtotal, 0, '', '.') . ")";        
        $productos2[] = array(
            "id" => $row["id"],
