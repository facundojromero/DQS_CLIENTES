...
if ($result->num_rows > 0) {
        $sql = "UPDATE cliente SET nombre='$nombre', apellido='$apellido', telefono='$telefono', telefono2='$telefono2', direccion='$direccion', provincia='$provincia', ciudad='$ciudad', cbu='$cbu', alias='$alias' , cbu_titular='$cbu_titular' WHERE user_id = $user_id"; // ¡Campo telefono2 agregado!
    if ($conn->query($sql) === TRUE) {
        echo "Actualizando datos ... ";
        header("refresh:1;url=?new=datos"); // redirige después de 1 segundos
        exit();
    } else {
        ...
        $sql = "INSERT INTO cliente (user_id, nombre, apellido, telefono, telefono2, direccion, provincia, ciudad, cbu_titular, cbu, alias) VALUES ($user_id, '$nombre', '$apellido', '$telefono', '$telefono2', '$direccion', '$provincia', '$ciudad', '$cbu_titular', '$cbu', '$alias')"; // ¡Campo telefono2 agregado!
        if ($conn->query($sql) === TRUE) {
