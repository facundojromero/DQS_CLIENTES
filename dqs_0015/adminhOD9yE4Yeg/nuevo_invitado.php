...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($totalInvitados !== $totalNombres) {
        ...
        $query_verificar = mysqli_query($conn, "SELECT COUNT(*) as total FROM pre_invitados WHERE codigo = '$codigo'");
        ...
        $query_insert = mysqli_query($conn, "INSERT INTO pre_invitados (nombre, apellido, acompanado, cantidad_mayores, cantidad_menores, id_prioridad, ingreso, fecha_registro, codigo) VALUES ('$nombre', '$apellido', '$acompanado', '$cantidad_mayores', '$cantidad_menores', '$id_prioridad', '$ingreso', now(), '$codigo')");
        if ($query_insert) {
            $id_invitado = mysqli_insert_id($conn);
            $nombre_invitado_a_insertar = !empty(trim($nombre_invitado)) ? $nombre_invitado : $nombre;

            $query_insert_nombre = mysqli_query($conn, "INSERT INTO pre_invitados_listado_mesa (id_invitados, nombre_invitado) VALUES ('$id_invitado', '$nombre_invitado_a_insertar')");
            ...
            foreach ($acompanante_nombres as $index => $nombre_acompanante) {
                if (!empty(trim($nombre_acompanante))) {
                    ...
                    $query_insert_acompanante = mysqli_query($conn, "INSERT INTO pre_invitados_listado_mesa (id_invitados, nombre_invitado, nombre2, apellido2) VALUES ('$id_invitado', '$apodo_acompanante_a_insertar', '$nombre_acompanante', '$apellido_acompanante')");
                }
            }
            
            foreach ($telefonos as $telefono) {
                if (!empty(trim($telefono))) {
                    $query_insert_tel = mysqli_query($conn, "INSERT INTO pre_invitados_tel (id_invitados, tel_enviar) VALUES ('$id_invitado', '$telefono')");
                }
            }
