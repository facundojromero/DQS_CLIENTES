...
dqs_require_admin_gifts_enabled($conn, 'html');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_gifts_display_settings') {
    $titulos = array(
        'titulo_transferencia_regalos' => $_POST['titulo_transferencia_regalos'] ?? '',
        'titulo_cuenta_pesos_regalos' => $_POST['titulo_cuenta_pesos_regalos'] ?? '',
        'titulo_cuenta_dolares_regalos' => $_POST['titulo_cuenta_dolares_regalos'] ?? '',
    );

    $defaultsTitulos = array(
    ...
    foreach ($titulos as $clave => $valor) {
        $titulos[$clave] = trim((string)$valor) !== '' ? trim((string)$valor) : $defaultsTitulos[$clave];
    }

    guardarConfiguracionVisualRegalos($conn, array(
        'mostrar_lista_regalos' => isset($_POST['mostrar_lista_regalos']) ? 1 : 0,
        'mostrar_transferencia_regalos' => isset($_POST['mostrar_transferencia_regalos']) ? 1 : 0,
        'titulo_transferencia_regalos' => $titulos['titulo_transferencia_regalos'],
        'titulo_cuenta_pesos_regalos' => $titulos['titulo_cuenta_pesos_regalos'],
        'titulo_cuenta_dolares_regalos' => $titulos['titulo_cuenta_dolares_regalos'],
    ));
...
<head>
    ...
    <style>
        ...
        .gifts-settings__header {
            margin-bottom: 20px;
        }
        .gifts-settings__header h3 {
            margin: 0 0 6px;
        }
        .gifts-settings__header p,
        .gifts-card__description {
            margin: 0;
            color: #667085;
            line-height: 1.5;
        }
        .gifts-settings__grid {
            display: grid;
        ...
        .gifts-settings__actions {
            display: flex;
            justify-content: flex-end;
            margin-top: 20px;
        }
        .gifts-settings__actions .action-btn {
            min-width: 210px;
        }
...
<body>
        ...
        <form method="POST" class="gifts-settings">
            <input type="hidden" name="action" value="save_gifts_display_settings">
            <div class="gifts-settings__header">
                <h3>Configuración de regalos para invitados</h3>
                <p>Elegí qué alternativas verán tus invitados y personalizá los títulos de la sección.</p>
            </div>

            <div class="gifts-settings__grid">
                <section class="gifts-card" aria-labelledby="gifts-status-title">
                    <h4 id="gifts-status-title">1. Estado de regalos</h4>
                    <p class="gifts-card__description">Activá o desactivá cada alternativa de forma independiente.</p>
                    <label class="gifts-toggle" for="mostrar_lista_regalos">
                        <span>Mostrar lista de regalos</span>
                        <input type="checkbox" id="mostrar_lista_regalos" name="mostrar_lista_regalos" value="1" <?php echo $configVisualRegalos['mostrar_lista_regalos'] ? 'checked' : ''; ?>>
                    </label>
                    <label class="gifts-toggle" for="mostrar_transferencia_regalos">
                        <span>Mostrar datos de transferencia</span>
                        <input type="checkbox" id="mostrar_transferencia_regalos" name="mostrar_transferencia_regalos" value="1" <?php echo $configVisualRegalos['mostrar_transferencia_regalos'] ? 'checked' : ''; ?>>
                    </label>
                    <label class="gifts-toggle" for="show_giftcard">
                        <span>Mostrar Gift Card</span>
                        <input type="checkbox" id="show_giftcard" name="show_giftcard" value="1" <?php echo $showGiftcard ? 'checked' : ''; ?>>
                    </label>
                </section>

                <section class="gifts-card" aria-labelledby="gifts-copy-title">
                    <h4 id="gifts-copy-title">2. Textos visibles para invitados</h4>
                    <p class="gifts-card__description">Este título presenta los datos de transferencia en la sección de regalos.</p>
                    <div class="gifts-field">
                        <label for="titulo_transferencia_regalos">Título de la sección de transferencias</label>
                        <input type="text" class="input-text" id="titulo_transferencia_regalos" name="titulo_transferencia_regalos" value="<?php echo htmlspecialchars($configVisualRegalos['titulo_transferencia_regalos']); ?>">
                    </div>
                </section>

                <section class="gifts-card" aria-labelledby="gifts-transfer-title">
                    <h4 id="gifts-transfer-title">3. Transferencias</h4>
                    <p class="gifts-card__description">Personalizá los encabezados de las cuentas. El titular, Alias y CBU/CVU se toman de tus datos actuales.</p>
                    <div class="gifts-account">
                        <h5>Datos en pesos</h5>
                        <div class="gifts-field">
                            <label for="titulo_cuenta_pesos_regalos">Título de la cuenta</label>
                            <input type="text" class="input-text" id="titulo_cuenta_pesos_regalos" name="titulo_cuenta_pesos_regalos" value="<?php echo htmlspecialchars($configVisualRegalos['titulo_cuenta_pesos_regalos']); ?>">
                        </div>
                    </div>
                    <div class="gifts-account">
                        <h5>Datos en dólares</h5>
                        <div class="gifts-field">
                            <label for="titulo_cuenta_dolares_regalos">Título de la cuenta</label>
                            <input type="text" class="input-text" id="titulo_cuenta_dolares_regalos" name="titulo_cuenta_dolares_regalos" value="<?php echo htmlspecialchars($configVisualRegalos['titulo_cuenta_dolares_regalos']); ?>">
                        </div>
                    </div>
                </section>

                <section class="gifts-card" aria-labelledby="gifts-summary-title">
                    <h4 id="gifts-summary-title">4. Vista y resumen</h4>
                    <p class="gifts-card__description">Tené en cuenta cómo se comportará el acceso a regalos.</p>
                    <ul class="gifts-help">
                        <li>Si solo mostrás transferencia, el botón Regalos llevará directamente a los datos de transferencia.</li>
                        <li>Si desactivás ambas opciones, la sección regalos no se mostrará.</li>
                    </ul>
                    <p id="giftsVisibilityWarning" style="display:none;" role="status">
                No hay ninguna opción de regalo visible para los invitados.
                    </p>
                </section>
            </div>

            <div class="gifts-settings__actions">
                <button type="submit" class="action-btn">Guardar configuración</button>
            </div>
        </form>
    ...
    <script>
        function actualizarAvisoRegalosVisibles() {
            const lista = document.getElementById('mostrar_lista_regalos');
            const transferencia = document.getElementById('mostrar_transferencia_regalos');
            const aviso = document.getElementById('giftsVisibilityWarning');
            if (!lista || !transferencia || !aviso) return;
            aviso.style.display = (!lista.checked && !transferencia.checked) ? 'block' : 'none';
        }
        document.addEventListener('DOMContentLoaded', () => {
            actualizarAvisoRegalosVisibles();
            document.getElementById('mostrar_lista_regalos')?.addEventListener('change', actualizarAvisoRegalosVisibles);
            document.getElementById('mostrar_transferencia_regalos')?.addEventListener('change', actualizarAvisoRegalosVisibles);
        });
