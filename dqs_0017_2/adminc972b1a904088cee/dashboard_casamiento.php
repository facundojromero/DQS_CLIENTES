...
// Filtros reutilizables para datos por persona (invitados_listado_mesa)
$listadoConditions = ["i.activo < 2"];
$listadoTypes = '';
$listadoParams = [];

if ($statusFiltro !== '') {
    $listadoConditions[] = "i.activo = ?";
    $listadoTypes .= 's';
    $listadoParams[] = $statusFiltro;
}
if ($confirmacionFiltro !== '') {
    if ($confirmacionFiltro === 'Si') {
        $listadoConditions[] = "lm.asiste = 1";
    } elseif ($confirmacionFiltro === 'No') {
        $listadoConditions[] = "lm.asiste = 0";
    } elseif ($confirmacionFiltro === 'NULL') {
        $listadoConditions[] = "lm.asiste IS NULL";
    }
}
if ($busquedaFiltro !== '') {
    $listadoConditions[] = "(lm.nombre_invitado LIKE ? OR lm.nombre2 LIKE ? OR lm.apellido2 LIKE ?)";
    $listadoTypes .= 'sss';
    $busquedaLike = "%$busquedaFiltro%";
    $listadoParams[] = $busquedaLike;
    $listadoParams[] = $busquedaLike;
    $listadoParams[] = $busquedaLike;
}
if ($ingresoFiltro !== '') {
    $listadoConditions[] = "i.ingreso = ?";
    $listadoTypes .= 's';
    $listadoParams[] = $ingresoFiltro;
}
if ($prioridadFiltro !== '') {
    $listadoConditions[] = "i.id_prioridad = ?";
    $listadoTypes .= 's';
    $listadoParams[] = $prioridadFiltro;
}

$whereListado = ' WHERE ' . implode(' AND ', $listadoConditions);
...
// KPI: Totales por persona (fuente de verdad: invitados_listado_mesa)
$sql_totales_listado = "
    SELECT
        COUNT(*) AS total_invitados,
        SUM(CASE WHEN lm.asiste = 1 THEN 1 ELSE 0 END) AS total_asiste,
        SUM(CASE WHEN lm.asiste = 0 THEN 1 ELSE 0 END) AS total_no_asiste,
        SUM(CASE WHEN lm.asiste IS NULL THEN 1 ELSE 0 END) AS total_pendientes,
        SUM(CASE WHEN lm.asiste IN (0,1) THEN 1 ELSE 0 END) AS total_confirmados
    FROM invitados_listado_mesa lm
    INNER JOIN invitados i ON i.id = lm.id_invitados
    $whereListado
";
$stmtTotalesListado = $conn->prepare($sql_totales_listado);
if ($stmtTotalesListado && !empty($listadoParams)) {
    $stmtTotalesListado->bind_param($listadoTypes, ...$listadoParams);
}
...
if ($stmtTotalesListado && $stmtTotalesListado->execute()) {
    $resultTotalesListado = $stmtTotalesListado->get_result();
    if ($resultTotalesListado) {
        $totalesListado = array_merge($totalesListado, $resultTotalesListado->fetch_assoc() ?: []);
    }
}

$total = (int)($totalesListado['total_invitados'] ?? 0);
...
// KPI: Total de personas confirmadas (asiste=1)
$confirmados = (int)($totalesListado['total_asiste'] ?? 0);

// KPI: Total de personas NO asistentes (asiste=0)
$no_confirmados = (int)($totalesListado['total_no_asiste'] ?? 0);

// KPI: Total de personas PENDIENTES de confirmación
$pendientes = (int)($totalesListado['total_pendientes'] ?? 0);

// KPI: Porcentaje de confirmación = total asiste (asiste=1) / total esperado
...
$sql_por_alimento = "
    SELECT lm.alimento AS categoria, COUNT(*) AS cantidad
    FROM invitados_listado_mesa lm
    INNER JOIN invitados i ON i.id = lm.id_invitados
    $whereListado
    AND lm.asiste IN (0,1)
    AND TRIM(COALESCE(lm.alimento, '')) <> ''
    GROUP BY lm.alimento
    ORDER BY cantidad DESC
";
$stmtPorAlimento = $conn->prepare($sql_por_alimento);
if ($stmtPorAlimento && !empty($listadoParams)) {
    $stmtPorAlimento->bind_param($listadoTypes, ...$listadoParams);
}

$por_alimento = false;
if ($stmtPorAlimento && $stmtPorAlimento->execute()) {
    $por_alimento = $stmtPorAlimento->get_result();
}
...
// Recaudación confirmada: cada regalo se toma una sola vez, en su primera confirmación.
// La fecha se agrupa en horario de Argentina sin modificar el valor almacenado en UTC.
$sql_recaudacion_confirmada = "
    SELECT
        DATE(DATE_SUB(c.confirm_date, INTERVAL 3 HOUR)) AS dia,
    ...
    GROUP BY DATE(DATE_SUB(c.confirm_date, INTERVAL 3 HOUR)), r.pago_con
    ORDER BY dia ASC, moneda_codigo ASC
";
$recaudacion_result = $conn->query($sql_recaudacion_confirmada);
$recaudacion_confirmada = [];
if ($recaudacion_result) {
    while ($row = $recaudacion_result->fetch_assoc()) {
        $recaudacion_confirmada[] = [
            'dia' => $row['dia'],
            'moneda' => (int)$row['moneda_codigo'] === 1 ? 'ARS' : 'USD',
            'pagos' => (int)$row['pagos_confirmados'],
            'total' => (float)$row['total_diario'],
        ];
    }
}

$sql_monedas_no_reconocidas = "
    ...
    WHERE r.activo = 1
      AND (r.pago_con IS NULL OR r.pago_con NOT IN (1, 2))
";
$monedas_no_reconocidas_result = $conn->query($sql_monedas_no_reconocidas);
$monedas_no_reconocidas = $monedas_no_reconocidas_result
    ? (int)($monedas_no_reconocidas_result->fetch_assoc()['cantidad'] ?? 0)
    : 0;
$recaudacion_json = json_encode(
    $recaudacion_confirmada,
    JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT
);
$hoy_argentina = gmdate('Y-m-d', time() - (3 * 60 * 60));
...
<h2>Resumen</h2>
    ...
    <div class="search-container">
        ...
        <div class="search-item">
            ...
            <select id="confirmationFilter" class="search-input" onchange="applyFilter()">
                ...
                <option value="NULL" <?php if ($confirmacionFiltro == 'NULL') echo 'selected'; ?>>Pendiente</option>
            </select>
        </div>
        <div class="search-item">
    ...
    <div class="kpi-row">
        ...
        <div class="col-md-4 col-lg-2">
            <a class="kpi-card-link" href="index.php?new=invitados&amp;confirmacion=Si" aria-label="Ver invitados confirmados">
        ...
        <div class="col-md-4 col-lg-2">
            <a class="kpi-card-link" href="index.php?new=invitados&amp;confirmacion=No" aria-label="Ver invitados que no asisten">
        ...
        <div class="col-md-4 col-lg-2">
            <a class="kpi-card-link" href="index.php?new=invitados&amp;confirmacion=NULL" aria-label="Ver invitados pendientes de confirmación">
...
<section class="revenue-section" aria-labelledby="revenueTitle">
    <div class="revenue-header">
        <div>
            <h3 id="revenueTitle">Recaudación confirmada</h3>
            <p>Importes diarios de regalos activos, según su primera confirmación.</p>
        </div>
        <label class="revenue-range" for="revenueRange">
            Período
    ...
    <?php if ($monedas_no_reconocidas > 0): ?>
        <p class="revenue-note" role="note"><i class="fas fa-info-circle" aria-hidden="true"></i> Hay operaciones confirmadas con moneda no reconocida que no se incluyen en el gráfico.</p>
    <?php endif; ?>
    <div class="revenue-grid">
        <article class="revenue-card">
            <div class="revenue-card-heading">
                <div><span>Recaudación diaria ARS</span><strong id="revenueTotalARS">$ 0</strong></div>
                <small id="revenuePaymentsARS">0 pagos confirmados</small>
            </div>
            <div class="revenue-chart"><canvas id="revenueChartARS"></canvas><p id="revenueEmptyARS" class="revenue-empty" hidden>No hay pagos confirmados en ARS para este período.</p></div>
        </article>
        <article class="revenue-card">
            <div class="revenue-card-heading">
                <div><span>Recaudación diaria USD</span><strong id="revenueTotalUSD">US$ 0,00</strong></div>
                <small id="revenuePaymentsUSD">0 pagos confirmados</small>
            </div>
            <div class="revenue-chart"><canvas id="revenueChartUSD"></canvas><p id="revenueEmptyUSD" class="revenue-empty" hidden>No hay pagos confirmados en USD para este período.</p></div>
        </article>
    </div>
</section>
...
<div class="chart-row">
    ...
    <script>
        const revenueRows = <?php echo $recaudacion_json ?: '[]'; ?>;
        const revenueToday = <?php echo json_encode($hoy_argentina); ?>;
        const revenueCharts = {};
        ...
        function renderRevenue(range) {
            let dates;
            if (range === 'all') {
                dates = [...new Set(revenueRows.map(row => row.dia))].sort();
            } else {
                dates = [];
                const today = new Date(`${revenueToday}T12:00:00`);
                for (let offset = Number(range) - 1; offset >= 0; offset--) {
            ...
            ['ARS', 'USD'].forEach(currency => {
                const rows = revenueRows.filter(row => row.moneda === currency && dates.includes(row.dia));
                const byDate = Object.fromEntries(rows.map(row => [row.dia, row]));
                const values = dates.map(date => byDate[date] ? Number(byDate[date].total) : 0);
                const total = rows.reduce((sum, row) => sum + Number(row.total), 0);
                const payments = rows.reduce((sum, row) => sum + Number(row.pagos), 0);
                const formatter = new Intl.NumberFormat('es-AR', {
                    style: 'currency', currency, maximumFractionDigits: currency === 'ARS' ? 0 : 2
                });
                document.getElementById(`revenueTotal${currency}`).textContent = formatter.format(total);
                document.getElementById(`revenuePayments${currency}`).textContent = `${payments} ${payments === 1 ? 'pago confirmado' : 'pagos confirmados'}`;

                const canvas = document.getElementById(`revenueChart${currency}`);
                const empty = document.getElementById(`revenueEmpty${currency}`);
                const hasData = payments > 0;
                canvas.hidden = !hasData;
                empty.hidden = hasData;
                if (revenueCharts[currency]) revenueCharts[currency].destroy();
                if (!hasData) return;

                revenueCharts[currency] = new Chart(canvas.getContext('2d'), {
                    type: 'bar',
                    data: {
                        labels: dates.map(date => new Intl.DateTimeFormat('es-AR', {day: '2-digit', month: '2-digit'}).format(new Date(`${date}T12:00:00`))),
                        datasets: [{data: values, backgroundColor: currency === 'ARS' ? '#7952b3' : '#198754', borderRadius: 5}]
                    },
                    options: {
                        responsive: true, maintainAspectRatio: false,
                        plugins: {legend: {display: false}, tooltip: {callbacks: {label: context => formatter.format(context.raw)}}},
                        scales: {y: {beginAtZero: true, ticks: {callback: value => formatter.format(value)}}, x: {grid: {display: false}}}
                    }
                });
            });
        }

        document.getElementById('revenueRange').addEventListener('change', event => renderRevenue(event.target.value));
        renderRevenue('30');
