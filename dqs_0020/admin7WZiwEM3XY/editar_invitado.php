...
$sql_nombres = "SELECT id, nombre_invitado, nombre2, apellido2, es_menor FROM invitados_listado_mesa WHERE id_invitados = ?";
...
$titular_es_menor_actual = 0;
$acompanantes_existentes = [];

if (!empty($nombres_listado)) {
    $titular = array_shift($nombres_listado);
    $nombre_invitado_principal = $titular['nombre_invitado'];
    $titular_es_menor_actual = (int)$titular['es_menor'];
    foreach ($nombres_listado as $acomp_row) {
        $acompanantes_existentes[] = [
            ...
            'es_menor' => (int)$acomp_row['es_menor']
        ];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $acompanante_es_menor = isset($_POST['acompanante_es_menor']) ? $_POST['acompanante_es_menor'] : [];
    $titular_es_menor = isset($_POST['titular_es_menor']) ? 1 : 0;
    ...
    $menoresSeleccionados = $titular_es_menor;
    foreach ($acompanante_nombres as $index => $nombre_comp) {
        if (!empty(trim($nombre_comp)) && isset($acompanante_es_menor[$index]) && (int)$acompanante_es_menor[$index] === 1) {
            $menoresSeleccionados++;
        }
    }

    if ($menoresSeleccionados !== (int)$cantidad_menores) {
        $_SESSION['mensaje'] = "La cantidad de menores debe coincidir con las personas marcadas como menor.";
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit();
    }
    ...
    // Actualizar la tabla 'invitados'
        ...
        $stmt_update_invitado->bind_param("ssiiisii", $nombre, $apellido, $acompanado, $cantidad_mayores, $cantidad_menores, $ingreso, $id_prioridad, $id_invitado_actual);
        ...
        $sql_insert_nombre_principal = "INSERT INTO invitados_listado_mesa (id_invitados, nombre_invitado, nombre2, apellido2, es_menor) VALUES (?, ?, ?, ?, ?)";
        $stmt_insert_nombre_principal = $conn->prepare($sql_insert_nombre_principal);
        $stmt_insert_nombre_principal->bind_param("isssi", $id_invitado_actual, $nombre_invitado_a_insertar, $nombre, $apellido, $titular_es_menor);
        ...
        foreach ($acompanante_nombres as $index => $nombre_acompanante) {
            if (!empty(trim($nombre_acompanante))) {
                ...
                $es_menor_acom = (isset($acompanante_es_menor[$index]) && (int)$acompanante_es_menor[$index] === 1) ? 1 : 0;
                $sql_insert_acompanante = "INSERT INTO invitados_listado_mesa (id_invitados, nombre_invitado, nombre2, apellido2, es_menor) VALUES (?, ?, ?, ?, ?)";
                $stmt_insert_acompanante = $conn->prepare($sql_insert_acompanante);
                $stmt_insert_acompanante->bind_param("isssi", $id_invitado_actual, $apodo_acompanante_a_insertar, $nombre_acompanante, $apellido_acompanante, $es_menor_acom);
...
<body>
    <div class="container">
        ...
        <form method="post" action="" class="formulario" onsubmit="return validarFormulario()">
            ...
            <div class="form-group">
                <label><input type="checkbox" name="titular_es_menor" value="1" <?php echo ($titular_es_menor_actual === 1) ? 'checked' : ''; ?>> Titular es menor</label>
            </div>
            ...
            <div id="acompanante-container">
                ...
                ?>
                    <div class="acompanante-row">
                        ...
                        <div class="form-group">
                            <label>Menor:</label>
                            <select name="acompanante_es_menor[]">
                                <option value="0" <?php echo ((int)$acompanante['es_menor'] === 0) ? 'selected' : ''; ?>>Mayor</option>
                                <option value="1" <?php echo ((int)$acompanante['es_menor'] === 1) ? 'selected' : ''; ?>>Menor</option>
                            </select>
                        </div>
                    </div>
                <?php
                ...
                ?>
                    <div class="acompanante-row">
                        ...
                        <div class="form-group">
                            <label>Menor:</label>
                            <select name="acompanante_es_menor[]">
                                <option value="0" selected>Mayor</option>
    ...
    <script>
        document.getElementById('addAcompanante').addEventListener('click', function() {
            ...
            var menorGroup = document.createElement('div');
            menorGroup.className = 'form-group';
            menorGroup.innerHTML = '<label>Menor:</label><select name="acompanante_es_menor[]"><option value="0" selected>Mayor</option><option value="1">Menor</option></select>';
            div.appendChild(menorGroup);
        ...
        function validarFormulario() {
            ...
            var menoresMarcados = document.querySelector('input[name="titular_es_menor"]')?.checked ? 1 : 0;
            var menoresSelect = document.querySelectorAll('select[name="acompanante_es_menor[]"]');
            acompananteNombres.forEach(function(input, index) {
                if (input.value.trim() && menoresSelect[index] && parseInt(menoresSelect[index].value || '0', 10) === 1) {
                    menoresMarcados++;
                }
            });

            if (menoresMarcados !== cantidadMenores) {
                alert('La cantidad de menores debe coincidir con las personas marcadas como menor.');
                return false;
            }
            return true;
        }
