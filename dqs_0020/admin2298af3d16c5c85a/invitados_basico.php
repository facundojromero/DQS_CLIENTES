...
if (isset($_GET['activadisimo']) && $_GET['activadisimo'] == 1) {
    ...
    echo "<script>
        window.open('invitaciones/index.php?id_invitados=" . $_GET['idd'] . "', '_blank', 'width=1,height=1,left=9999,top=9999');
    </script>";
}
...
if (!$id && (!isset($_GET['nuevo']) || $_GET['nuevo'] != '0')): ?>
 ...
 if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        // Ruta fija y correcta a invitacion.php (ajusta la ruta base según tu estructura)
        $invitacion_php = $protocolo . $host . "/regalos_casamiento_v2/invitacion.php";

        $nombre_img = "{$row['id_imagen']}.jpg";
        $token = urlencode(cifrar($nombre_img));
        $invitacion_url = $invitacion_php . "?k=" . $token;

        $mensaje = urlencode("Hola {$row['invitados']}, te estamos esperando 🎉 Mirá tu invitación: $invitacion_url");
